---
title: "MCP servers"
description: "Tool servers give Humboldt abilities beyond the machine. The ones your account comes with connect at sign-in. Add your own in a few lines."
---
...
```
/mcp              # every server: pick one to show its tools, reconnect it, sign in or sign out
...
```
hum mcp add jira https://.../mcp --header "Authorization: Bearer $TOKEN"
hum mcp add db -e DB_URL=postgres://... -- npx db-mcp --readonly
hum mcp login jira      # sign in in the browser; Hum keeps what the server issued
hum mcp list            # what is declared here, and what you are signed in to
hum mcp get jira        # the entry, as TOML
hum mcp remove jira
```

A url is reached over HTTP; a command after `--` is started here. Both write a `[[mcp]]` table in your own config; `--scope project` writes the repository's `.hum/config.toml` instead, so everyone who works in it gets the server, on the person's word. In a session, `/mcp` lists them and `/mcp <name>` goes straight to one: show its tools, reconnect it, sign in or sign out.
...
| `env` | Variables the server's process is given |
